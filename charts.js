...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultant_array = samples.filter(sampledObj => sampledObj.id == sample);

    // 3.1. Create a variable that filters the metadata array for the object with the desired sample number.
    var meta = data.metadata;
    var resulting_array = meta.filter(s_Obj => s_Obj.id == sample);

    // 3.2. Create a variable that holds the first sample in the metadata array.
    var resulting_sample = resulting_array[0];

    //  5. Create a variable that holds the first sample in the array.
    var result_sample = resultant_array[0];

    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otu_ids = result_sample.otu_ids;
    var otu_labels = result_sample.otu_labels;
    var sample_values = result_sample.sample_values;

    // 3.3. Create a variable that holds the washing frequency.
    var w_freq = parseFloat(resulting_sample.wfreq);
